...
'''
Here is the code which actually runs when you call this script, first it loads in your director csv data, then it initiates a jsonl file 'new_credits.jsonl' and, if the file exists, it checks and records which directors are already present in the data and prints their ids. 
'''
print("\nInitiating Jeff's IMDB scraping process. For each director inputted in the initial csv, this script will collect credits with all crew members for each director's feature films. If you would like to filter out specific roles or jobs, input a list of strings of roles you would like to include.")
# prompt for user input
datapath = input("\nEnter path for director information csv: ") or "100_film_directors.csv"
jsonl_file = input("\nEnter name and path of JSONL file: ") or "scraped_credits.jsonl"
validroles = input("\nEnter a comma-separated list of valid roles for directors: ") or ['']
...
print(dir_ids_in_jsonl) #report which directors are already found in the data, they will not be scraped again
